...
class renderer extends plugin_renderer_base {
    ...
    public function render_main(main $main) : string {
        ...
        $template = get_config('local_community', 'templatetype');
        $path = $CFG->dirroot . '/blocks/vitrina/templates/' . $template . '/main.mustache';

        if ($template != 'default' && file_exists($path)) {
            $templatefile = 'local_community/' . $template . '/main';
        } else {
            $templatefile = 'local_community/main';
        }
    ...
    public function render_catalog(catalog $catalog) : string {
        ...
        if ($template != 'default' && file_exists($path)) {
            $templatefile = 'local_community/' . $template . '/catalog';
        } else {
            $templatefile = 'local_community/catalog';
        }
    ...
    public function render_detail(detail $detail) : string {
        ...
        if ($template != 'default' && file_exists($path)) {
            $templatefile = 'local_community/' . $template . '/detail';
        } else {
            $templatefile = 'local_community/detail';
        }
    ...
    public function render_course(object $course) : string {
        ...
        if ($template != 'default' && file_exists($path)) {
            $templatefile = 'local_community/' . $template . '/course';
        } else {
            $templatefile = 'local_community/course';
        }
